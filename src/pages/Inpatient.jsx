...
export default function Inpatient() {
  ...
  const [inpatientTreatments, setInpatientTreatments] = useState([]);
  ...
  const [showPlaceModal, setShowPlaceModal] = useState(null);
  ...
  const [isEditingPatient, setIsEditingPatient] = useState(null);
  ...
  const [dischargeInvoice, setDischargeInvoice] = useState(null);
  ...
  async function loadAll() {
    try {
      const rm = await db.query('SELECT * FROM rooms WHERE isActive = 1 ORDER BY floor, name');
      setRooms(rm);
      setDoctors(await db.query('SELECT * FROM doctors WHERE isActive = 1'));
      
      const activePats = await db.query(`
         SELECT rp.*, p.fullName as patientName, d.fullName as doctorName, r.name as roomName, 
                r.pricePerDay as defaultRoomPrice, r.mealPrice as roomMealPrice, 
                rt.name as tariffName, rt.pricePerDay as tariffPrice
         FROM room_patients rp
         LEFT JOIN patients p ON rp.patientId = p.id
         LEFT JOIN doctors d ON rp.doctorId = d.id
         LEFT JOIN rooms r ON rp.roomId = r.id
         LEFT JOIN room_tariffs rt ON rp.tariffId = rt.id
  ...
  async function loadTreatments() {
    ...
    try {
      const trms = await db.query(`
        SELECT t.*, p.fullName as patientName, r.name as roomName
        FROM inpatient_treatments t
        JOIN room_patients rp ON t.roomPatientId = rp.id
  ...
  useEffect(() => { if (roomPatients.length > 0) loadTreatments(); }, [roomPatients]);

  function formatPrice(n) { return Number(n || 0).toLocaleString('uz-UZ'); }

  const handleDragStart = (e, rp) => {
    setDraggedPatient(rp);
    e.dataTransfer.setData('text/plain', rp.id);
  };
  const handleDragOver = (e) => e.preventDefault();
  const handleDrop = async (e, targetRoom, targetBedIndex) => {
    e.preventDefault();
    ...
    try {
      ...
      await loadAll();
    } catch (err) { toast.error("O'tkazishda xatolik: " + err.message); }
    setDraggedPatient(null);
  };
  ...
  async function loadVitals(rpId) {
    try {
      const res = await db.query('SELECT * FROM vital_signs WHERE roomPatientId = ? ORDER BY createdAt DESC', [rpId]);
  ...
  async function toggleTreatmentDone(treatmentId, isDone) {
    try {
  ...
  async function generateDischargeInvoice(rp) {
    const entryDate = new Date(rp.entryDate);
    const autoDays = Math.max(1, Math.ceil((new Date() - entryDate) / (1000 * 60 * 60 * 24)));
    ...
    setDischargeInvoice({ rp, days, roomPrice, roomTotal, mealPrice, mealTotal, additionalDebt, grandTotal, deposit, toPay });
  ...
  async function confirmDischarge() {
    ...
    try {
      await db.update('room_patients', rp.id, { 
        status: 'discharged', exitDate: new Date().toISOString(), days, totalCost: grandTotal 
      });
      if (deposit > 0) {
  ...
  const extractKitchenData = () => {
    const data = { total: 0, byDiet: {}, byRoom: [] };
  ...
  const floors = Object.keys(groupedRooms).map(Number).sort((a,b)=>b-a);

  return (
    <div className="page page-inpatient">
      <div className="page-header">
        <h1>🏥 Statsionar Maxsus</h1>
        <div className="page-header-actions"><button className="btn btn-secondary" onClick={() => loadAll()}>🔄 Yangilash</button></div>
      </div>

      <div className="tabs mb-4" style={{borderBottom:'1px solid var(--border-color)', paddingBottom:15, display:'flex', gap:10}}>
        <button className={`btn btn-sm ${activeTab === 'map' ? 'btn-primary' : 'btn-ghost'}`} onClick={()=>setActiveTab('map')}>🗺️ Xonalar xaritasi</button>
        <button className={`btn btn-sm ${activeTab === 'nurse' ? 'btn-primary' : 'btn-ghost'}`} onClick={()=>setActiveTab('nurse')}>👩‍⚕️ Hamshira posti</button>
        <button className={`btn btn-sm ${activeTab === 'kitchen' ? 'btn-primary' : 'btn-ghost'}`} onClick={()=>setActiveTab('kitchen')}>🍲 Oshxona</button>
        
        {activeTab === 'map' && (
          <div style={{marginLeft:'auto', display:'flex', gap:10}}>
             <select className="form-input" style={{width:130, padding:4}} onChange={(e)=>setFilterType(e.target.value)}>
               <option value="all">Barcha xonalar</option>
               <option value="luxury">🌟 Lyuks</option>
               <option value="shared">🛌 Common</option>
             </select>
             <select className="form-input" style={{width:130, padding:4}} onChange={(e)=>setFilterStatus(e.target.value)}>
                      ...
                      {floorRooms.map(room => {
                        ...
                        return (
                          <div key={room.id} className="card p-3" style={{border:`1px solid ${room.cleaningStatus === 'cleaning'?'var(--accent-warning)':'var(--border-color)'}`, position:'relative', cursor:'pointer'}} onClick={() => { setPlaceForm(p=>({...p, patientName:'', phone:'', age:'', gender:'m', doctorId:'', bedIndex:'', deposit:'0', notes:'', arrivalTime: new Date().toLocaleTimeString('uz-UZ').slice(0,5)})); setShowPlaceModal(room); }}>
                           ...
                           <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid var(--border-color)', paddingBottom:8, marginBottom:10}}>
                             <div style={{fontWeight:800, fontSize:18}}>{room.name} <span style={{fontSize:12, fontWeight:400, color:'var(--text-muted)'}}>{occupants.length}/{room.capacity} sig'im</span></div>
                           ...
                           <div className="bed-grid">
                             {Array.from({length: room.capacity}).map((_, i) => {
                               ...
                               if (occupant) {
                                  const days = Math.max(1, Math.ceil((new Date() - new Date(occupant.entryDate)) / (1000 * 60 * 60 * 24)));
                                  ...
                                  return (
                                    <div key={i} className={`bed-card ${occupant.status === 'reserved'?'bed-reserved':'bed-occupied'} ${lowDeposit && occupant.status !== 'reserved'?'bed-deposit-danger':''}`} draggable onDragStart={e=>handleDragStart(e, occupant)}>
                                      ...
                                      <div style={{display:'flex', justifyContent:'space-between', marginTop:4, gap:4}}>
                                        ...
                                        <button className="btn btn-sm btn-ghost" onClick={()=>setShowDepositModal(occupant)}>💵</button>
                                        <button className="btn btn-sm btn-danger" onClick={()=>generateDischargeInvoice(occupant)}>🚪</button>
                                      </div>
                                    </div>
                                  );
                               } else {
                                  ...
                                  return (
                                    <div key={i} className={`bed-card ${isCleaning ? 'bed-cleaning' : 'bed-empty'}`} onDragOver={handleDragOver} onDrop={e=>handleDrop(e, room, bedLabel)}>
                                      <div className="bed-status-dot"></div>
                                      <div className="bed-title">{bedLabel} {room.type === 'luxury' && '🌟'}</div>
                                      <div style={{fontSize:11, color:'var(--text-muted)', marginTop:10, textAlign:'center'}}>{isCleaning ? '🧼 Tozalamoqda' : '🟢 Bo\'sh'}</div>
                                      {isCleaning && <button className="btn btn-sm btn-ghost mt-auto" style={{fontSize:9, padding:2}} onClick={async()=>{await db.update('rooms', room.id, { cleaningStatus:'clean' }); loadAll();}}>Tayyor ✅</button>}
                                    </div>
                                  );
                               }
                             })}
          ...
          <div className="nurse-station-grid">
            <div className="nurse-column">
              <div className="nurse-column-header">
                ...
                <select className="form-input btn-ghost" style={{fontSize:10, padding:2}} value={filterNurseStatus} onChange={e=>setFilterNurseStatus(e.target.value)}>
                  <option value="all">Hammasi</option>
            ...
            <div className="nurse-column" style={{background:'transparent', border:'none'}}>
               <div className="card glass-card mb-4" style={{borderLeft:'4px solid var(--accent-danger)'}}>
                  <div className="card-header"><span style={{color:'var(--accent-danger)'}}>🆘 SOS Chaqiruvlar</span></div>
                  <div className="card-body">
                     <div className="sos-card">204-xona: Bemor isitma (Emergency!)</div>
                     <div style={{fontSize:12, textAlign:'center', color:'var(--text-muted)'}}>Boshqa chaqiruvlar yo'q.</div>
                  </div>
               </div>
               <div className="card glass-card" style={{flex:1}}>
                  <div className="card-header">Smena topshirish (Handover)</div>
                  <div className="card-body">
                    <textarea className="form-input w-full" rows={8} placeholder="Keyingi smenaga izohlar..." value={handoverNote} onChange={e=>setHandoverNote(e.target.value)} style={{fontSize:13}}></textarea>
                    <button className="btn btn-primary w-full mt-3" onClick={()=>toast.success("Eslatma saqlandi")}>Saqlash</button>
                  </div>
               </div>
            </div>
          </div>
        ...
        {activeTab === 'kitchen' && (
          <div style={{display:'grid', gridTemplateColumns:'1fr 2fr', gap:20}}>
            <div className="card glass-card">
              <div className="card-header"><h3>🍲 Umumiy Hisobot</h3></div>
              <div className="card-body">
                  <div style={{fontSize:48, fontWeight:800, textAlign:'center', color:'var(--accent-primary)', marginBottom:20}}>{kitchenData.total} <span style={{fontSize:18}}>ta</span></div>
                  <div style={{display:'flex', flexDirection:'column', gap:10}}>
                    {Object.entries(kitchenData.byDiet).map(([key, count]) => (
                      <div key={key} style={{display:'flex', justifyContent:'space-between', padding:10, background:'var(--bg-input)', borderRadius:8}}>
                        <span>{DIET_TYPES[key] || key}</span><span style={{fontWeight:700}}>{count} ta</span>
                      </div>
                    ))}
                  </div>
                  <button className="btn btn-primary w-full mt-5" onClick={()=>window.print()}>🖨️ Chiqarish</button>
              </div>
            </div>
            <div className="card glass-card">
              <div className="card-header"><h3>📄 Xonama-xona ro'yxat</h3></div>
              <div className="card-body p-0" style={{maxHeight:500, overflow:'auto'}}>
                  <table className="data-table">
                    <thead><tr><th>Joy</th><th>Bemor</th><th>Parhez</th></tr></thead>
                    <tbody>
                      {kitchenData.byRoom.map(({rp, dietLabel}, i) => (
      ...
      <Modal isOpen={Boolean(showPlaceModal)} onClose={()=>setShowPlaceModal(null)} title="Joylashtirish">
        <div className="form-grid">
          <div className="form-group full-width"><label>Bemor F.I.Sh</label><input className="form-input" placeholder="To'liq ismi..." value={placeForm.patientName} onChange={e=>setPlaceForm({...placeForm, patientName:e.target.value})}/></div>
          <div className="form-group"><label>Telefon</label><input className="form-input" placeholder="+998..." value={placeForm.phone} onChange={e=>setPlaceForm({...placeForm, phone:e.target.value})}/></div>
          <div className="form-group"><label>Yosh</label><input type="number" className="form-input" placeholder="30" value={placeForm.age} onChange={e=>setPlaceForm({...placeForm, age:e.target.value})}/></div>
          <div className="form-group"><label>Jins</label>
            <select className="form-input" value={placeForm.gender} onChange={e=>setPlaceForm({...placeForm, gender:e.target.value})}>
              <option value="m">Erkak</option><option value="f">Ayol</option>
            </select>
          </div>
          ...
          <div className="form-group"><label>Depozit (so'm)</label><input type="number" className="form-input" value={placeForm.deposit} onChange={e=>setPlaceForm({...placeForm, deposit:e.target.value})}/></div>
          <div className="form-group"><label>Holat</label>
            <select className="form-input" value={placeForm.status || 'active'} onChange={e=>setPlaceForm({...placeForm, status:e.target.value})}>
          ...
          {placeForm.hasMeals && <div className="form-group full-width"><label>Parhez</label><select className="form-input" value={placeForm.dietType} onChange={e=>setPlaceForm({...placeForm, dietType:e.target.value})}>{Object.entries(DIET_TYPES).map(([k,v]) => <option key={k} value={k}>{v}</option>)}</select></div>}
        ...
        <button className="btn btn-primary w-full mt-4" onClick={async()=>{
          ...
          try {
            ...
            let patientId;
            const existing = await db.query('SELECT id FROM patients WHERE fullName = ? LIMIT 1', [placeForm.patientName]);
            if (existing.length > 0) {
              ...
              const p = await db.insertReturn('patients', {
                ...
                birthDate: placeForm.age ? String(new Date().getFullYear() - Number(placeForm.age)) : null,
              });
              patientId = p.id;
            }
      ...
      <Modal isOpen={Boolean(showTreatmentModal)} onClose={()=>setShowTreatmentModal(null)} title="💉 Muolaja">
         <div className="form-grid">
           <div className="form-group full-width"><label>Nomi</label><input className="form-input" value={treatmentForm.title} onChange={e=>setTreatmentForm({...treatmentForm, title:e.target.value})}/></div>
           <div className="form-group"><label>Vaqti</label><input type="time" className="form-input" value={treatmentForm.scheduledTime} onChange={e=>setTreatmentForm({...treatmentForm, scheduledTime:e.target.value})}/></div>
           <div className="form-group full-width"><label>Izoh</label><input className="form-input" value={treatmentForm.notes} onChange={e=>setTreatmentForm({...treatmentForm, notes:e.target.value})}/></div>
         </div>
         <button className="btn btn-primary w-full mt-4" onClick={saveTreatment}>O'rnatish</button>
      </Modal>

      <Modal isOpen={Boolean(showDepositModal)} onClose={()=>setShowDepositModal(null)} title="💵 Depozit">
      ...
      <Modal isOpen={Boolean(showDischargeModal)} onClose={()=>setShowDischargeModal(null)} title="🚪 Chiqarish">
      ...
      <Modal isOpen={Boolean(showVitalsModal)} onClose={()=>setShowVitalsModal(null)} title={`💓 Monitoring: ${showVitalsModal?.patientName}`}>
         <div className="form-grid">
           <div className="form-group"><label>Harorat</label><input type="number" step="0.1" className="form-input" value={vitalsForm.temperature} onChange={e=>setVitalsForm({...vitalsForm, temperature:e.target.value})}/></div>
           <div className="form-group"><label>Bosim</label><input className="form-input" value={vitalsForm.bp} onChange={e=>setVitalsForm({...vitalsForm, bp:e.target.value})}/></div>
           <div className="form-group"><label>Puls</label><input type="number" className="form-input" value={vitalsForm.pulse} onChange={e=>setVitalsForm({...vitalsForm, pulse:e.target.value})}/></div>
           <div className="form-group"><label>Vazn</label><input type="number" className="form-input" value={vitalsForm.weight} onChange={e=>setVitalsForm({...vitalsForm, weight:e.target.value})}/></div>
         </div>
         <button className="btn btn-primary w-full mt-4" onClick={saveVitals}>Saqlash</button>
      </Modal>
