...
export default function JournalPage({ onNavigate }){
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (filters.category) data = data.filter(e => (e.category || '').toLowerCase().includes(filters.category.toLowerCase()))
  ...
  const footerTotals = useMemo(() => {
    ...
    const cash  = filteredEntries.filter(e => e.payment_type === 'cash').reduce((a, e) => a + (Number(e.amount) || 0), 0)
    const card  = filteredEntries.filter(e => e.payment_type === 'card').reduce((a, e) => a + (Number(e.amount) || 0), 0)
    const transfer = filteredEntries.filter(e => e.payment_type === 'transfer').reduce((a, e) => a + (Number(e.amount) || 0), 0)
    const debt  = filteredEntries.filter(e => e.payment_type === 'debt').reduce((a, e) => a + (Number(e.amount) || 0), 0)
    return { total, cash, card, transfer, debt, count: filteredEntries.length, patients: new Set(filteredEntries.map(e => e.patient_name)).size }
  }, [filteredEntries])
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const data = { amount: Number(editForm.amount), discount: Number(editForm.discount), paymentType: editForm.paymentType }
  ...
  async function handleZReport() {
    const confirm = window.confirm('Diqqat! Bugungi smenani yopish va Z-Report chiqarishni tasdiqlaysizmi?')
    if (!confirm) return
  ...
  function renderRow(e, isChild = false) {
    ...
    return (
      <tr key={`${e.type}-${e.id}${isChild ? '-c' : ''}`} style={{ 
        fontSize: 15, height: 65, borderBottom: '1px solid var(--border-color)', ...rowStyle
      }}>
        <td style={{ fontWeight:700 }}>{dt.toLocaleDateString('uz-UZ')}</td>
        <td style={{ opacity:0.6, fontSize:13 }}>{dt.toLocaleTimeString('uz-UZ', { hour:'2-digit', minute:'2-digit' })}</td>
        ...
        <td style={{ textAlign:'right' }}>
          {docShare > 0 ? (
            <div style={{ display:'flex', alignItems:'center', justifyContent:'flex-end', gap:8 }}>
              <span style={{ color:'var(--accent-warning)', fontWeight: 800 }}>{formatPrice(docShare)}</span>
            </div>
          ) : '—'}
        </td>
        <td style={{ textAlign:'center' }}>{e.room_name ? <span className="badge badge-info">{e.room_name}</span> : '—'}</td>
        <td style={{ textAlign:'right', fontWeight:900, color: e.type==='expense' ? 'var(--accent-warning)' : 'var(--accent-primary)', fontSize: 18 }}>
          {e.type==='expense' ? '-' : ''}{!isChild && e._group ? formatPrice(e._groupTotal) : formatPrice(e.amount)}
        </td>
        <td>
          <span 
        ...
        <td>
          <div style={{ display:'flex', gap:6 }}>
            <button className="btn-icon-link" title="Tahrirlash" onClick={() => { setEditingEntry(e); setEditForm({amount:String(e.amount), discount:String(e.discount||0), paymentType:e.payment_type}) }}>✏️</button>
            <button className="btn-icon-link" title="Chek chiqarish" onClick={() => window.print()}>🖨️</button>
            {e.payment_type === 'debt' && (
              <button className="btn-icon-link text-success" title="Qarzni yopish" onClick={() => setPayingDebt(e)}>💰</button>
            )}
          </div>
        </td>
      </tr>
  ...
  return (
    <div className="page page-journal" style={{ height:'100%', display:'flex', flexDirection:'column', gap: 20 }}>
      {/* HEADER */}
      ...
      {/* FINTECH WIDGETS */}
      <div className="stats-grid no-print" style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:20, flexShrink:0 }}>
        {[
          ...
          <div key={i} className="card glass-card" style={{ padding: 24, display:'flex', flexDirection:'column', gap:10, borderLeft: `6px solid ${s.color}` }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 900, color: s.color }}>{formatPrice(s.value)} <small style={{ fontSize:14 }}>so'm</small></div>
      ...
      {/* TABLE AREA */}
      <div className="card glass-card" style={{ flex:1, overflow:'hidden', display:'flex', flexDirection:'column' }}>
        <div className="card-body" style={{ overflow:'auto', padding:0 }}>
          <table className="data-table" style={{ width:'100%', minWidth:1600, borderCollapse: 'separate', borderSpacing: 0 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={14} className="p-10 text-center">{t('common.loading')}</td></tr>
        ...
        {payingDebt && (
          <div className="form-grid p-4">
            ...
            <div className="form-group full-width" style={{ marginTop: 20 }}>
              ...
              <select className="form-input" value={debtPayForm.paymentType} onChange={e => setDebtPayForm({paymentType: e.target.value})}>
                <option value="cash">💵 Naqd pul</option>
                <option value="card">💳 Plastik karta</option>
                <option value="transfer">📲 Click / Payme</option>
              </select>
            </div>
            <div className="form-actions" style={{ marginTop:30, display:'flex', gap:10, justifyContent:'stretch' }}>
              <button className="btn btn-primary" style={{ flex: 1, padding: 15 }} onClick={handlePayDebt}>✅ TO'LANDI</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* EDIT MODAL */}
      <Modal isOpen={!!editingEntry} onClose={() => setEditingEntry(null)} title="Tahrirlash">
        {editingEntry && (
          <div className="form-grid p-4">
             <div className="form-group full-width">
               <label>Summa</label>
               <input type="number" className="form-input" value={editForm.amount} onChange={e => setEditForm({...editForm, amount: e.target.value})} />
             </div>
