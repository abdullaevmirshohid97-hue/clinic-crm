...
export default function Settings() {
  ...
  async function loadAll() {
    try {
      ...
      setServices(await db.query('SELECT * FROM services ORDER BY orderIndex, name'));
  ...
  async function saveClinicSettings() {
    try {
      for (const [key, value] of Object.entries(clinic)) {
        await db.query('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', [key, value]);
      }
  ...
  async function savePrinter(name) {
    ...
    try {
      await db.query('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)', ['printerName', name]);
      toast.success(t('common.success'));
  ...
  async function togglePermission(role, feature, currentState) {
    ...
    try {
      await db.run("INSERT OR REPLACE INTO permissions (id, role, feature, canAccess) VALUES ((SELECT id FROM permissions WHERE role=? AND feature=?), ?, ?, ?)", 
        [role, feature, role, feature, newVal]);
      await loadAll();
  ...
  return (
    <div className="page page-settings" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', flex: 1, gap: 20, overflow: 'hidden' }}>
        {/* Sidebar Tabs */}
        <div className="settings-sidebar" style={{ width: 250, borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', gap: 10, paddingRight: 15 }}>
          <div className={`filter-tab ${activeTab === 'profile' ? 'active' : ''}`} onClick={() => setActiveTab('profile')}>🏥 Klinika profili</div>
          <div className={`filter-tab ${activeTab === 'staff' ? 'active' : ''}`} onClick={() => setActiveTab('staff')}>👥 Xodimlar va Huquqlar</div>
          <div className={`filter-tab ${activeTab === 'finance' ? 'active' : ''}`} onClick={() => setActiveTab('finance')}>💸 Moliya va Cheklar</div>
          <div className={`filter-tab ${activeTab === 'system' ? 'active' : ''}`} onClick={() => setActiveTab('system')}>⚙️ Tizim va Xavfsizlik</div>
          <div className={`filter-tab ${activeTab === 'audit' ? 'active' : ''}`} onClick={() => {setActiveTab('audit'); loadAuditLogs();}}>📜 Audit Jurnali</div>
        </div>
        ...
        <div className="settings-content" style={{ flex: 1, overflowY: 'auto', paddingRight: 10 }}>
            ...
            <div className="settings-section">
              {showSection(['klinika', 'profil', 'manzil', 'telefon', 'shior']) && (
                <div className="card glass-card mb-4" style={{borderLeft:'4px solid var(--accent-primary)'}}>
                  <div className="card-header"><h3>🏥 Klinika ma'lumotlari</h3></div>
          ...
          {activeTab === 'system' && (
             <div className="settings-section">
                {showSection(['xavfsizlik', 'parol', 'pin']) && (
                  <div className="card glass-card mb-4">
                    <div className="card-header"><h3>🔐 Xavfsizlik (Admin PIN)</h3></div>
                    <div className="card-body">
          ...
          {activeTab === 'audit' && (
             <div className="settings-section">
                <div className="card glass-card mb-4">
                  <div className="card-header">
                    <h3>📜 Harakatlar Jurnali (Audit Log)</h3>
                  </div>
                  <div className="card-body p-0">
                    <table className="data-table">
                       <thead>
