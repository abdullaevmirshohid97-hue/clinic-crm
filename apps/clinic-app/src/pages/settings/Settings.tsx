...
export default function Settings() {
  ...
  async function loadAuditLogs() {
    try {
      ...
      rows.sort(
        (a, b) =>
          new Date(
            (b as AuditLog & { timestamp?: string }).timestamp || b.createdAt || ''
          ).getTime() -
          new Date(
            (a as AuditLog & { timestamp?: string }).timestamp || a.createdAt || ''
          ).getTime()
      );
  ...
  async function handleTestPrint() {
    try {
      await printReceipt({
        ...
        services: [{ name: 'Test Xizmat', price: 50000 }],
        total: 50000,
        paymentType: 'cash',
      });
  ...
  function handleQrFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toast.error('Fayl 2MB dan oshmasligi kerak');
      return;
    }
    setQrFile(file);
  ...
  async function deleteDoctor(id: number | string) {
    try {
  ...
  function openServiceModal(svc: ClinicService | null = null) {
    if (svc) {
      ...
      setEditService(null);
      setServiceForm({ name: '', price: '', category: '', duration: '' });
    }
  ...
  async function deleteService(id: number | string) {
    try {
  ...
  async function handleTelegramTest() {
    ...
    try {
      const res = await window.electronAPI?.invoke?.(
        'backup:testTelegram',
        clinic.telegramBotToken,
        clinic.telegramChatId
      );
    ...
    >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Klinika nomi, aloqa, manzil, rekvizitlar
            </div>
          </button>
          ...
          <button
            ...
            style={{
              border: 'none',
              background: activeTab === 'staff' ? 'var(--bg-input)' : 'transparent',
              textAlign: 'left',
              padding: '14px 16px',
              borderRadius: 12,
              cursor: 'pointer',
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Rollarni chegaralash, xodimlar royxati
            </div>
          </button>
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Xizmat narxlari, printer va kvitansiya logikasi
            </div>
          </button>
          ...
          <button
            ...
            style={{
              border: 'none',
              background: activeTab === 'system' ? 'var(--bg-input)' : 'transparent',
              textAlign: 'left',
              padding: '14px 16px',
              borderRadius: 12,
              cursor: 'pointer',
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Pin-kodlar, Zaxiralash, Telegram auto-backup
            </div>
          </button>
          ...
          <button
            ...
            style={{
              border: 'none',
              background: activeTab === 'audit' ? 'var(--bg-input)' : 'transparent',
              textAlign: 'left',
              padding: '14px 16px',
              borderRadius: 12,
              cursor: 'pointer',
          ...
          >
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Admin/SaaS platforma xavfsizlik audit loglari
            </div>
          </button>
        </div>
        ...
        <div className="settings-content" style={{ flex: 1, overflowY: 'auto', paddingRight: 10 }}>
          ...
          {activeTab === 'profile' && (
            <div className="settings-section" style={{ paddingLeft: 10 }}>
              <div style={{ marginBottom: 30 }}>
                <h2 style={{ fontSize: 28, fontWeight: 900, marginBottom: 5 }}>
                  Tashkilot Profili
                </h2>
                <p style={{ color: 'var(--text-muted)' }}>
                  Klinikaning asosiy ma'lumotlari, brending va rekvizitlari.
                </p>
              </div>
              {showSection(['klinika', 'profil', 'manzil', 'telefon', 'shior']) && (
                ...
                >
                  <div
                    ...
                    style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: 15 }}
                  >
                  ...
                  <div className="card-body">
                    <div className="form-grid">
                      ...
                      <div className="form-group">
                        ...
                        <input
                          className="form-input"
                          value={clinic.clinicSlogan}
                          onChange={(e) => setClinic({ ...clinic, clinicSlogan: e.target.value })}
                        />
                      </div>
                    </div>
                    <button className="btn btn-primary mt-3" onClick={saveClinicSettings}>
                      💾 Saqlash
                    </button>
                  </div>
                </div>
          ...
          {activeTab === 'staff' && (
            <div className="settings-section" style={{ paddingLeft: 10 }}>
              <div style={{ marginBottom: 30 }}>
                <h2 style={{ fontSize: 28, fontWeight: 900, marginBottom: 5 }}>
                  Xodimlar va Huquqlar (ACL)
                </h2>
                <p style={{ color: 'var(--text-muted)' }}>
                  SaaS xavfsizlik modeli, modullarga kirish huquqlarini chegaralash va komissiyalar
                  konfiguratsiyasi.
                </p>
              </div>
              {showSection(['shifokor', 'xodim', 'komissiya']) && (
                ...
                >
                  ...
                  >
                    ...
                    <button className="btn btn-sm btn-primary" onClick={() => openDoctorModal()}>
                      + Shifokor
                    </button>
                  </div>
                      ...
                      <tbody>
                        {doctors.map((d) => (
                          <tr key={d.id}>
                            ...
                            <td>
                              <button className="btn-icon-only" onClick={() => openDoctorModal(d)}>
                                ✏️
                              </button>
                            </td>
                          </tr>
                ...
                <div className="card glass-card mb-4">
                  ...
                  <div className="card-body">
                    <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 15 }}>
                      Har bir rol qaysi modullarga kira olishini belgilang. (Admin barcha
                      ruxsatlarga ega).
                    </p>
          ...
          {activeTab === 'finance' && (
            <div className="settings-section" style={{ paddingLeft: 10 }}>
              <div style={{ marginBottom: 30 }}>
                <h2 style={{ fontSize: 28, fontWeight: 900, marginBottom: 5 }}>
                  Uskunalar va Cheklar
                </h2>
                <p style={{ color: 'var(--text-muted)' }}>
                  Kvitansiya dizayner interfeysi, termal printerlarni ulash hamda xizmatlar
                  narxnomasi.
                </p>
              </div>
                      ...
                      <tbody>
                        {services.slice(0, 10).map((s) => (
                          <tr key={s.id}>
                            <td>{s.name}</td>
                            <td>{s.category || '—'}</td>
                            <td>{formatPrice(s.price)} so'm</td>
                            <td>
                              <button className="btn-icon-only" onClick={() => openServiceModal(s)}>
                                ✏️
                              </button>
                            </td>
                          </tr>
                        ...
                        {services.length > 10 && (
                          <tr>
                            <td
                              colSpan={4}
                              style={{ textAlign: 'center', fontSize: 11, opacity: 0.6 }}
                            >
                              ... va yana {services.length - 10} ta xizmat
                            </td>
                          </tr>
                        )}
                      </tbody>
              ...
              {showSection(['chek', 'dizayn', 'printer', 'qr', 'kvitansiya']) && (
                <div
                  className="card glass-card mb-4"
                  style={{ borderLeft: '4px solid var(--accent-success)' }}
                >
                  <div className="card-header">
                    <h3>📝 Chek dizayneri va Printer</h3>
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button className="btn btn-sm btn-ghost" onClick={loadPrinters}>
                        🔄 {t('settings.selectPrinter')}
                      </button>
                      <button className="btn btn-sm btn-secondary" onClick={handleTestPrint}>
                        🖨️ Test Chek
                      </button>
                    </div>
                  </div>
                  <div className="card-body">
                    <div className="form-grid mb-4">
                      ...
                      <div className="form-group">
                        ...
                        <select
                          className="form-input"
                          value={clinic.receiptWidth}
                          onChange={(e) => setClinic({ ...clinic, receiptWidth: e.target.value })}
                        >
                          <option value="58">Tor (58mm)</option>
                          <option value="80">Keng (80mm)</option>
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="form-grid">
                      <div className="form-group full-width">
                        ...
                        <textarea
                          className="form-input"
                          rows={2}
                          value={clinic.receiptHeader}
                          onChange={(e) => setClinic({ ...clinic, receiptHeader: e.target.value })}
                          placeholder="Masalan: Klinikada bugun aksiya..."
                        ></textarea>
                      </div>
                      <div className="form-group full-width">
                        ...
                        <textarea
                          className="form-input"
                          rows={2}
                          value={clinic.receiptFooter}
                          onChange={(e) => setClinic({ ...clinic, receiptFooter: e.target.value })}
                          placeholder="Masalan: Shifokor qabuliga navbat kuting..."
                        ></textarea>
                      </div>
                    </div>
                      ...
                      <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 10 }}>
                            Chekning quyi qismida rasmiy sayt yoki Telegram botga olib chiquvchi JPG
                            QR yuklang. (Faqat JPG, max 1MB).
                          </p>
                    ...
                    <button className="btn btn-primary mt-4" onClick={saveClinicSettings}>
                      💾 Dizayn va sozlamalarni saqlash
                    </button>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'system' && (
            <div className="settings-section" style={{ paddingLeft: 10 }}>
              <div style={{ marginBottom: 30 }}>
                ...
                <p style={{ color: 'var(--text-muted)' }}>
                  Lokal xavfsizlik cheklovlari (PIN kodlar) va Bulutli Avtomatik zaxiralash tizimi.
                </p>
              </div>
                  ...
                  <div className="card-body">
                    <div className="form-group full-width" style={{ maxWidth: 300 }}>
                      <input
                        ...
                        className="form-input"
                        value={adminPin}
                        placeholder="PIN o'rnatish..."
                        onChange={(e) => setAdminPin(e.target.value)}
                      />
                      <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                        <button className="btn btn-primary" onClick={saveAdminPin}>
                          Saqlash
                        </button>
                        <button
                ...
                <div className="card glass-card mb-4" style={{ borderLeft: '4px solid #0088CC' }}>
                  ...
                  <div className="card-body">
                    <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 20 }}>
                      Ma'lumotlar bazasini har kuni avtomatik nusxalash va uni xavfsiz tarzda
                      Telegram orqali yuborish mexanizmi.
                    </p>
                    ...
                    <div className="form-grid mb-4">
                      <div className="form-group">
                        ...
                        <input
                          type="time"
                          className="form-input"
                          value={clinic.backupTime}
                          onChange={(e) => setClinic({ ...clinic, backupTime: e.target.value })}
                        />
                      </div>
                      <div className="form-group full-width">
                        <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          * Dastur fonda ishlab turganda belgilangan vaqtda bazani arxivlaydi.
                        </p>
                      </div>
                    </div>
                    ...
                    >
                      <h4 style={{ color: '#0088CC', marginBottom: 10 }}>
                        ✈️ Telegram Bulutli Sinxronizatsiya
                      </h4>
                      <div className="form-grid">
                        <div className="form-group full-width">
                          ...
                          <input
                            className="form-input"
                            type="password"
                            value={clinic.telegramBotToken}
                            onChange={(e) =>
                              setClinic({ ...clinic, telegramBotToken: e.target.value })
                            }
                        ...
                        <div className="form-group full-width">
                          ...
                          <input
                            className="form-input"
                            value={clinic.telegramChatId}
                            onChange={(e) =>
                              setClinic({ ...clinic, telegramChatId: e.target.value })
                            }
                      ...
                      <div style={{ display: 'flex', gap: 10, marginTop: 15 }}>
                        <button className="btn btn-primary" onClick={saveClinicSettings}>
                          Sozlamalarni saqlash
                        </button>
                        <button className="btn btn-secondary" onClick={handleTelegramTest}>
                          📤 Telegram'ga Test Yuborish
                        </button>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        ...
                        >
                          📤 Hozirgi holatni eksport qilish
                        </button>
                        <button
                          className="btn btn-ghost"
                        ...
                        >
                          📥 Tiklash (Barcha eski ma'lumotlar o'chadi)
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          {activeTab === 'audit' && (
            <div className="settings-section" style={{ paddingLeft: 10 }}>
              <div style={{ marginBottom: 30 }}>
                ...
                <p style={{ color: 'var(--text-muted)' }}>
                  Dastur ichidagi barcha modifikatsiya (kiritish, tahrirlash, o'chirish) harakatlari
                  kancelyariyasi.
                </p>
              </div>
              <div
                      ...
                      {auditLogs.map((log, i) => (
                        <tr key={i}>
                          <td style={{ fontSize: 12 }}>
                            {new Date(log.timestamp || log.createdAt || '').toLocaleString('uz-UZ')}
                          </td>
                          <td>
                            <span className="badge badge-outline">
                              {log.userName || log.userId || 'Tizim / Admin'}
                            </span>
                          </td>
                          <td
                            style={{
      ...
      >
        <div className="form-grid">
          ...
          <div className="form-group">
            <label>Ulush (Komissiya %)</label>
            <input
              type="number"
              className="form-input"
              value={doctorForm.commission_rate}
              onChange={(e) =>
                setDoctorForm({ ...doctorForm, commission_rate: Number(e.target.value) })
              }
            />
          </div>
        </div>
        <div className="form-actions mt-4">
          {editDoctor && (
            <button className="btn btn-danger" onClick={() => deleteDoctor(editDoctor.id)}>
              🗑️ O'chirish
            </button>
          )}
      ...
      <Modal
        ...
        title={editService ? 'Xizmatni tahrirlash' : 'Yangi xizmat'}
      >
        <div className="form-grid">
          <div className="form-group full-width">
        ...
        <div className="form-actions mt-4">
          {editService && (
            <button className="btn btn-danger" onClick={() => deleteService(editService.id)}>
              🗑️ O'chirish
            </button>
          )}
          <span style={{ flex: 1 }}></span>
