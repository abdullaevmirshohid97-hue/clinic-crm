...
export default function Analytics() {
  ...
  async function loadPharmStats() {
    ...
    try {
      ...
      if (pharmPeriod === 'hour') jFromTime = now.getTime() - 3600000;
        ...
        jFromTime = new Date(pharmFrom).getTime();
      ...
      const journal = allJnl
        .filter((j) => {
          ...
          if (pharmPeriod === 'custom' && pharmTo && ct > new Date(pharmTo + 'T23:59:59').getTime())
            return false;
          return true;
        })
  ...
  async function loadStats() {
    ...
    try {
      ...
      setStatsionarStats({
        ...
        avgStay:
          dischargedRp.length > 0
            ? Math.round(
                dischargedRp.reduce((s, rp) => s + (rp.days || 0), 0) / dischargedRp.length
              )
            : 0,
        mealRevenue: 0,
      });
      ...
      if (retPct < 20)
        insightsList.push({
          type: 'warn',
          text: `Qayta keluvchi bemorlar atigi ${retPct.toFixed(0)}%.`,
          color: 'var(--accent-warning)',
        });
      if (totalRevenue > 0 && totalRevenue > prevTxs.reduce((s, t) => s + (t.amount || 0), 0) * 1.1)
        insightsList.push({
          type: 'up',
          text: 'Tushum sezirarli darajada oshdi! 🚀',
  ...
  const statCards = [
    {
      icon: '💰',
      label: t('analytics.dailyRevenue') || 'Umumiy tushum',
      value: formatPrice(stats.totalRevenue),
      color: 'var(--accent-primary)',
      sub: t('analytics.period'),
    },
    {
      icon: '🏥',
      label: t('analytics.statsionarRevenue') || 'Statsionar tushumi',
      value: formatPrice(statsionarRevenue),
      color: 'var(--accent-warning)',
      sub: 'Inpatient',
    },
    {
      icon: '🩺',
      label: t('analytics.serviceRevenue') || 'Xizmat tushumi',
      value: formatPrice(serviceRevenue),
      color: 'var(--accent-info)',
      sub: 'Outpatient',
    },
    {
      icon: '👥',
      label: t('analytics.totalPatients') || 'Bemorlar soni',
    ...
    {
      icon: '🎫',
      label: t('analytics.averageCheck') || "O'rtacha chek",
    ...
    {
      icon: '📉',
      label: t('analytics.expenses') || 'Xarajatlar',
    ...
    {
      icon: '💊',
      label: 'Dorixona foydasi',
      value: formatPrice(pharmData.totalProfit),
      color: '#06b6d4',
      sub: 'Soffoyda',
    },
    {
      icon: '📈',
      label: t('analytics.netProfit') || 'Klinika sof foydasi',
      value: formatPrice(netProfit + (pharmData.totalProfit || 0)),
      color: 'var(--accent-success)',
      sub: 'Jami',
    },
  ];
    ...
    <div className="page page-analytics">
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <button
            className={`btn ${activeTab === 'statsionar' ? 'btn-primary' : 'btn-ghost'}`}
            onClick={() => setActiveTab('statsionar')}
          >
            🏥 {t('inpatient.title')}
          </button>
          <button
          ...
          <button
            className={`btn ${activeTab === 'payout_history' ? 'btn-primary' : 'btn-ghost'}`}
            onClick={() => setActiveTab('payout_history')}
          >
            📜 {t('common.history') || 'Tarix'}
          </button>
          <button
            ...
            style={
              activeTab !== 'pharmacy'
                ? {
                    background:
                      'linear-gradient(135deg,rgba(124,58,237,0.15),rgba(79,70,229,0.12))',
                    borderColor: '#7c3aed',
                    color: '#7c3aed',
                  }
                : {}
      ...
      <div className="page-body">
        {activeTab === 'overview' ? (
          <>
            ...
            >
              {statCards.map((card, i) => (
                <div
                  key={i}
                  className="card glass-card stat-card-v2"
                  style={{
                    padding: '20px',
                    borderRadius: '16px',
                    position: 'relative',
            ...
            {comparisonData.previous > 0 &&
              (() => {
                ...
                const ptsPct =
                  comparisonData.previousPatients > 0
                    ? Math.round((ptsDelta / comparisonData.previousPatients) * 100)
                    : 0;
                return (
                  <div
                    ...
                    style={{
                      ...
                      borderLeft:
                        revDelta >= 0
                          ? '5px solid var(--accent-success)'
                          : '5px solid var(--accent-danger)',
                    }}
                  >
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
                      📊 O'tgan davrga nisbatan solishtirma tahlil
                    </div>
                    <div
                      style={{
                        display: 'grid',
                      ...
                      <div>
                        ...
                        <div
                          style={{
                            fontSize: 28,
                            fontWeight: 800,
                            color: revDelta >= 0 ? 'var(--accent-success)' : 'var(--accent-danger)',
                          }}
                        >
                          {revDelta >= 0 ? '📈' : '📉'} {revPct >= 0 ? '+' : ''}
                          {revPct}%
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                          {formatPrice(comparisonData.previous)} →{' '}
                          {formatPrice(comparisonData.current)}
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                        ...
                        <div
                          style={{
                            fontSize: 28,
                            fontWeight: 800,
                            color: ptsDelta >= 0 ? 'var(--accent-success)' : 'var(--accent-danger)',
                          }}
                        >
                          {ptsDelta >= 0 ? '📈' : '📉'} {ptsPct >= 0 ? '+' : ''}
                          {ptsPct}%
                  ...
                  {doctorStats.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
                      {doctorStats.map((d, i) => (
                        ...
                        >
                          <div style={{ minWidth: 140, fontWeight: 600, fontSize: 13 }}>
                            {d.fullName}
                          </div>
                          <div
                            style={{
                          ...
                          <span className="badge badge-info" style={{ fontSize: 10, minWidth: 60 }}>
                            {d.cnt} qabul
                          </span>
                        </div>
                      ))}
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span
                                  style={{
                                    ...
                                    height: 8,
                                    borderRadius: '50%',
                                    background: paymentColor(p.paymentType),
                                  }}
                                ></span>
                                <span>{paymentLabel(p.paymentType)}</span>
                              </div>
                              <span style={{ fontWeight: 700 }}>
                                {((p.revenue / totalPaymentRevenue) * 100).toFixed(0)}%
                              </span>
                            </div>
                          ))}
                        </div>
            ...
            {debtStats.total > 0 && (
              <div
                className="card glass-card"
                style={{ borderLeft: '5px solid var(--accent-danger)' }}
              >
                <div className="card-header">
                ...
                >
                  ...
                  >
                    <div
                      style={{ fontSize: 24, fontWeight: 'bold', color: 'var(--accent-danger)' }}
                    >
                      {formatPrice(debtStats.total)} {t('common.sum')}
                    </div>
                  ...
                  >
                    <div style={{ fontSize: 24, fontWeight: 'bold' }}>
                      {formatPrice(debtStats.avg)} {t('common.sum')}
                    </div>
                    <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 5 }}>
                      {t('analytics.avgDebtAmount')}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div>
                              ...
                              >
                                <div
                                  style={{
                                    ...
                                    height: 12,
                                    borderRadius: 3,
                                    background: pieCatColors[i % pieCatColors.length],
                                    flexShrink: 0,
                                  }}
                                ></div>
            ...
            <div className="card glass-card">
              <div className="card-header">
                <h3>📣 Bemor Kelish Manbalari (ROI)</h3>
              </div>
              <div className="card-body">
                {sourceData.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-muted)' }}>
                    📌 Hali bemorlarning kelish manbasi kiritilmagan. Qabulxonadan yangi bemor
                    ro'yxatdan o'tkazilganda "Qayerdan eshitdi?" maydonini to'ldiring.
                  </div>
                ) : (
                    ...
                    {sourceData.map((s, i) => {
                      ...
                      return (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <span
                            style={{ fontSize: 11, color: 'var(--text-secondary)', minWidth: 90 }}
                          >
                            O'rtacha: {formatPrice(avgPerPt)}
                          </span>
                        </div>
                      );
                    })}
          ...
          <div>
            <div className="card glass-card">
              <div className="card-header">
                <h3>🔥 Yuklama Xaritasi (Oxirgi 30 kun)</h3>
              </div>
          ...
          <div>
            ...
            <div className="card glass-card" style={{ marginBottom: 30 }}>
              <div className="card-header">
                <h3>👥 Bemorlar Sodiqlik Tahlili</h3>
              </div>
              <div className="card-body">
                {(() => {
                  const total = retentionData.newPatients + retentionData.returningPatients;
                  const retPct =
                    total > 0 ? ((retentionData.returningPatients / total) * 100).toFixed(1) : 0;
                  const newPct =
                    total > 0 ? ((retentionData.newPatients / total) * 100).toFixed(1) : 0;
                  return (
                    <div
                      style={{
                        display: 'grid',
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <div
                          style={{
                            ...
                            background:
                              total > 0
                                ? `conic-gradient(var(--accent-success) 0% ${retPct}%, var(--accent-primary) ${retPct}% 100%)`
                                : 'var(--bg-input)',
                          }}
                        >
                          <div
                      ...
                      <div>
                        <div style={{ display: 'flex', gap: 30, marginBottom: 20 }}>
                          ...
                          >
                            ...
                            >
                              {retentionData.newPatients}
                            </div>
                            <div
                              style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}
                            >
                              🆕 Birinchi marta ({newPct}%)
                            </div>
                          </div>
                        </div>
                        <div
            ...
            <div className="card glass-card">
              <div className="card-header">
                <h3>📣 Reklama Kanallari ROI</h3>
              </div>
              <div className="card-body p-0">
                {sourceData.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-muted)' }}>
                    📌 Hali bemorlarning kelish manbasi kiritilmagan.
                  </div>
                ) : (
          ...
          <div className="statsionar-pro-view">
            ...
            >
              <div
                className="card glass-card"
                style={{ padding: 25, borderTop: '4px solid var(--accent-warning)' }}
              >
                <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
                  {t('analytics.statsionarIncome')}
                </div>
                <div style={{ fontSize: 28, fontWeight: 'bold', marginTop: 8 }}>
                  {formatPrice(statsionarStats.totalRevenue)} {t('common.sum')}
                </div>
              </div>
              <div
                className="card glass-card"
                style={{ padding: 25, borderTop: '4px solid var(--accent-info)' }}
              >
                <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
                  {t('analytics.recoveredPatients')}
                </div>
                <div style={{ fontSize: 28, fontWeight: 'bold', marginTop: 8 }}>
                  {statsionarStats.patientCount} {t('analytics.units')}
                </div>
              </div>
              <div
                className="card glass-card"
                style={{ padding: 25, borderTop: '4px solid var(--accent-success)' }}
              >
                <div style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
                  {t('analytics.avgStayDuration')}
                </div>
                <div style={{ fontSize: 28, fontWeight: 'bold', marginTop: 8 }}>
                  {statsionarStats.avgStay} {t('analytics.day')}
                </div>
              </div>
            </div>
            ...
            <div className="card glass-card mb-5">
              <div className="card-header">
                <h3>🏢 {t('analytics.departmentAnalysis')}</h3>
              </div>
              <div className="card-body p-0">
                      ...
                      {statsionarDetails.length === 0 ? (
                        ...
                        statsionarDetails.map((dt, i) => (
                          <tr key={i}>
                            ...
                            <td>
                              {dt.totalPatients} {t('analytics.units')}
                            </td>
                            <td
                              style={{
                                textAlign: 'right',
                            ...
                            >
                              {formatPrice(dt.revenue)} {t('common.sum')}
                            </td>
                          </tr>
                        ))
                      )}
            ...
            <div className="card glass-card">
              <div className="card-header">
                ...
                <span className="badge badge-info">
                  {payoutHistory.length} {t('analytics.units')} {t('queue.journal')}
                </span>
              </div>
              <div className="card-body p-0">
                <div className="table-wrapper">
                        ...
                        payoutHistory.map((ph) => (
                          <tr key={ph.id}>
                            ...
                            <td style={{ fontWeight: '700', color: 'var(--accent-success)' }}>
                              {formatPrice(ph.amount)} {t('common.sum')}
                            </td>
                            <td>{ph.percentage}%</td>
                            ...
                            <td>
                              <button
                                ...
                                onClick={() => handleDeletePayout(ph.id)}
                              >
                                ✕
                              </button>
                            </td>
              ...
              <div
                ...
                style={{ padding: 20, textAlign: 'right', background: 'rgba(255,255,255,0.02)' }}
              >
          ...
          <div className="payout-view">
            <div className="card glass-card">
              <div className="card-header">
                ...
                <span className="badge badge-info">
                  {doctorPayouts.length} {t('common.all')}
                </span>
              </div>
              <div className="card-body p-0">
                <div className="table-wrapper">
                    ...
                    <tbody>
                      {doctorPayouts.length === 0 ? (
                        ...
                        doctorPayouts.map((dp) => {
                          const doctorShare = Math.round(
                            (dp.totalRevenue * (dp.commission_rate || 0)) / 100
                          );
                          return (
                            <tr key={dp.id}>
                              <td>
                              ...
                              <td>{dp.specialty}</td>
                              <td style={{ textAlign: 'right' }}>
                                {formatPrice(dp.totalRevenue)} {t('common.sum')}
                              </td>
                              <td style={{ textAlign: 'center' }}>
                                <span className="badge badge-primary" style={{ fontWeight: 700 }}>
                                  {dp.commission_rate || 0}%
                                </span>
                              </td>
                              <td
                                style={{
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setShowDoctorModal({
                                        ...
                                        cnt: dp.appointmentCount,
                                      });
                                      setPayoutForm({
                                        ...payoutForm,
                                        percentage: dp.commission_rate || 0,
                                      });
                                    }}
                                  >
                                    💸 {t('analytics.payment')}
              ...
              >
                <div style={{ fontSize: 20 }}>
                  {t('analytics.totalPaid')}:{' '}
                  <span
                    style={{ fontWeight: 'bold', color: 'var(--accent-success)', marginLeft: 15 }}
                  >
                    {formatPrice(
                      doctorPayouts.reduce(
                        (s, dp) => s + (dp.totalRevenue * (dp.commission_rate || 0)) / 100,
                        0
                      )
                    )}{' '}
                    {t('common.sum')}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginTop: 10 }}>
            ...
            >
              ...
              {[
                ...
                <button
                  key={opt.val}
                  onClick={() => setPharmPeriod(opt.val)}
                  style={{
                    padding: '7px 16px',
                    borderRadius: 20,
                    fontSize: 12,
            ...
            >
              {[
                ...
                {
                  icon: '📈',
                  label: 'Davr sof foyda',
                  val: pharmData.totalProfit,
                ...
                >
                  {pharmData.daily.map((d, i) => (
                    <div
                      ...
                      title={`${d.d}: ${Number(d.revenue).toLocaleString()} so'm`}
                    >
                      <div
                        style={{
            ...
            {/* ── Top/Slow Sellers */}
            <div
              style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 24 }}
            >
                    ...
                    <div key={i}>
                      ...
                      >
                        <div
                          style={{
                            height: '100%',
                            width: `${(s.soldQty / (pharmData.topSellers[0]?.soldQty || 1)) * 100}%`,
                            background: '#ef4444',
                          }}
                        />
                      </div>
                    ...
                    <div key={i}>
                      ...
                      >
                        <div
                          style={{
                            height: '100%',
                            width: `${(s.soldQty / (pharmData.slowSellers[0]?.soldQty || 1)) * 100}%`,
                            background: '#6366f1',
                          }}
                        />
                      </div>
              ...
              <div className="form-grid">
                <div className="form-group full-width">
                  ...
                  >
                    ...
                    >
                      ...
                      <span>
                        {showDoctorModal.cnt} {t('analytics.units')}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="form-group full-width">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPayoutForm({ ...payoutForm, percentage: Number(e.target.value) })
                    }
                  />
                </div>
                <div
                ...
                >
                  ...
                  >
                    {formatPrice(share)} {t('common.sum')}
                  </div>
                </div>
                <div className="form-group full-width">
                  ...
                  <textarea
                    className="form-input"
                    rows={2}
                    placeholder={t('analytics.notePlaceholder')}
      ...
      <Modal
        ...
        title={t('analytics.doctorJournal')}
        width={800}
      >
        {showDetailsModal && (
          <div className="details-view-container">
            <div className="details-header" style={{ marginBottom: 20 }}>
              ...
              <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>
                {period.toUpperCase()} — {t('analytics.serviceList')}
              </p>
            </div>
            <div className="table-wrapper" style={{ maxHeight: 400, overflow: 'auto' }}>
              <table className="data-table">
                    ...
                    doctorAppointments.map((da) => (
                      <tr key={da.id}>
                        <td style={{ fontSize: 12 }}>
                          {new Date(da.createdAt).toLocaleString('uz-UZ').slice(0, 16)}
                        </td>
                        <td>{da.patientName || t('analytics.unknown')}</td>
                        <td>{da.serviceName}</td>
                        <td>
                          <span
                            className={`badge ${da.paymentType === 'cash' ? 'badge-success' : 'badge-info'}`}
                          >
                            {da.paymentType}
                          </span>
                        </td>
                        <td style={{ textAlign: 'right', fontWeight: 600 }}>
                          {formatPrice(da.transactionAmount)}
                        </td>
                      </tr>
                    ))
                  )}
            ...
            >
              <div style={{ fontSize: 16, fontWeight: 600 }}>
                {t('reception.total')}:{' '}
                {formatPrice(
                  doctorAppointments.reduce((s, a) => s + (a.transactionAmount || 0), 0)
                )}{' '}
                {t('common.sum')}
              </div>
              <button
      ...
      <Modal
        ...
        title="📅 Davriy Xarajatlar (Jurnal)"
        width={1000}
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 20 }}>
          <div className="card glass-card p-4">
            ...
            <button className="btn btn-danger w-full mt-4" onClick={saveExpense}>
              💾 Saqlash
            </button>
          </div>
