...
const Reception = forwardRef<any, any>(function Reception(props, ref) {
  ...
  async function searchPatients(query: string) {
    ...
    try {
      ...
      let matched = allPat.filter(p => (p.fullName && p.fullName.toLowerCase().includes(q)) || (p.phone && p.phone.includes(q))).slice(0, 20);
  ...
  async function handleCloseShift() {
    ...
    try {
      ...
      const statsObj = shiftTxs.reduce((acc, t) => {
  ...
  async function handleSaveAndPrint() {
    ...
    try {
      ...
      if (selectedDoctor) {
        ...
        const todayQs = qs.filter(x => x.doctorId === selectedDoctor.id && x.createdAt && x.createdAt.startsWith(today));
        const nextNum = todayQs.length > 0 ? Math.max(...todayQs.map(x => x.number || 0)) + 1 : 1;
