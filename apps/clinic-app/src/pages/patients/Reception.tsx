...
const Reception = forwardRef<ReceptionHandle, Record<string, never>>(
  function Reception(_props, ref) {
    ...
    async function searchPatients(query: string) {
      ...
      try {
        ...
        const allR = await db.getAllRows('rooms');

        matched = matched.map((p) => {
          const activeRp = allRp.find((rp) => rp.patientId === p.id && rp.status === 'active');
          if (activeRp) {
            const room = allR.find((r) => r.id === activeRp.roomId);
            return { ...p, activeRoomId: activeRp.id, roomName: room ? room.name : '' };
          }
          return p;
        });
    ...
    async function startManualShift(type: string) {
      try {
        ...
        toast.success(
          `${type === 'day' ? t('reception.dayShift') : t('reception.nightShift')} ${t('common.success')}`
        );
      } catch (e: unknown) {
    ...
    async function handleCloseShift() {
      ...
      try {
        const txs = await db.getAllRows('transactions');
        const shiftTxs = txs.filter((t) => t.shift_id === activeShift.id);

        const statsObj = shiftTxs.reduce(
          (acc, t) => {
            if (acc[t.paymentType] !== undefined) {
              acc[t.paymentType] += t.amount || 0;
            }
            return acc;
          },
        ...
        await db.update('shifts', activeShift.id, {
          ...
          totalCash: statsObj.cash,
          totalCard: statsObj.card,
          totalTransfer: statsObj.transfer,
          totalDebt: statsObj.debt,
        });
    ...
    useEffect(() => {
      if (showDoctorModal && doctorGridRef.current) {
        ...
        const target = items[highlightedDoctorIndex] as HTMLElement;
        if (target) {
    ...
    async function handleSaveAndPrint() {
      ...
      try {
        ...
        for (const svc of selectedServices) {
          ...
          const maps = await db.getAllRows('consumables_mapping');
          const svcMaps = maps.filter((m) => m.serviceId === svc.id);
          if (svcMaps.length > 0) {
            const invList = await db.getAllRows('inventory');
            for (const m of svcMaps) {
              const totalQty = m.quantity * svc.quantity;
              const item = invList.find((i) => i.id === m.itemId);
              if (item) {
                await db.update('inventory', item.id, {
                  currentStock: item.currentStock - totalQty,
                });
          ...
          servicesForPrint.push({
            ...svc,
            name: svc.quantity > 1 ? `${svc.name} x${svc.quantity}` : svc.name,
            price: svc.price * svc.quantity,
          });
        }
      ...
      try {
        ...
        for (const svc of selectedServices) {
          ...
          for (const m of svcMaps) {
            ...
            const item = invList.find((i) => i.id === m.itemId);
            if (item) {
              await db.update('inventory', item.id, { currentStock: item.currentStock - totalQty });
              await db.insert('inventory_transactions', {
                itemId: m.itemId,
                type: 'exit',
                quantity: totalQty,
    ...
    return (
      <div className="page page-reception">
        <div className="page-header">
          ...
          <div className="page-header-actions">
            ...
            >
              {activeShift ? (
                <span
                  className={`badge ${activeShift.type === 'day' ? 'badge-success' : 'badge-warning'}`}
                >
                  {activeShift.type === 'day'
                    ? `☀️ ${t('reception.dayShift')}`
                    : `🌙 ${t('reception.nightShift')}`}
                </span>
              ) : (
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      {patientStatsionarInfo && (
                        <span className="badge badge-warning" style={{ marginRight: 10 }}>
                          {t('reception.statsionarBadge')} ({patientStatsionarInfo})
                        </span>
                      )}
                      <button
                ...
                <div className="card-header">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div className="btn-group btn-group-sm">
                      <button
                        className={`btn-icon-only ${serviceSortOrder === 'az' ? 'active' : ''}`}
                        onClick={() =>
                          setServiceSortOrder(serviceSortOrder === 'az' ? 'manual' : 'az')
                        }
                        title={t('reception.sortAZ')}
                      >
                        🔤
                      </button>
                      <button
                        className={`btn-icon-only ${serviceViewMode === 'grid' ? 'active' : ''}`}
                        onClick={() => setServiceViewMode('grid')}
                        title={t('reception.gridView')}
                      >
                        🔳
                      </button>
                      <button
                        className={`btn-icon-only ${serviceViewMode === 'list' ? 'active' : ''}`}
                        onClick={() => setServiceViewMode('list')}
                        title={t('reception.listView')}
                      >
                        📝
                      </button>
                    </div>
                ...
                <div className="card-body">
                  {services.length === 0 ? (
                    <p className="empty-state">
                      {t('common.noData')} — {t('settings.addService')}
                    </p>
                  ) : filteredServices.length === 0 ? (
                    <p className="empty-state">
                      "{serviceSearch}" — {t('reception.notFound')}
                    </p>
                  ) : (
                    <div
                      className={`${serviceViewMode === 'grid' ? 'service-select-grid' : 'service-select-list'} service-scroll-grid`}
                      ref={serviceGridRef}
                    >
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="service-item-price">
                              {formatPrice(svc.price)} {t('common.sum')}
                            </div>
                            {sel && <div className="service-qty-badge">x{sel.quantity}</div>}
                            <div
                              className="service-item-actions"
                    ...
                    <div className="service-item">
                      <div className="service-item-info">
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
                          ...
                          {selectedDoctor.prefix ? `[${selectedDoctor.prefix}]` : ''}
                        </div>
                      </div>
                      <button
                  ...
                  <div className="card-body">
                    <div className="service-list">
                      {selectedServices.map((svc) => (
                        <div
                          key={svc.id}
                          className={`service-item ${highlightedServiceId === svc.id ? 'highlighted' : ''}`}
                          onClick={() => setHighlightedServiceId(svc.id)}
                        >
                          <div className="service-item-info">
                            <div className="service-item-name">{svc.name}</div>
                            <div className="service-item-price">
                              ...
                              {formatPrice(svc.price * svc.quantity)} {t('common.sum')}
                            </div>
                          </div>
                ...
                <div className="card-body">
                  ...
                  <div className="discount-row">
                    ...
                    {discountAmount > 0 && (
                      <span style={{ color: 'var(--accent-danger)', fontWeight: 600 }}>
                        -{formatPrice(discountAmount)} {t('common.sum')}
                      </span>
                    )}
                  </div>
                  <div className="form-group" style={{ marginTop: 15 }}>
                    ...
                    <textarea
                      ...
                      placeholder={t('reception.noteEnter')}
                  ...
                  <div className="payment-total" style={{ marginTop: 15 }}>
                    ...
                    <span className="total-amount">
                      {formatPrice(finalAmount)} {t('common.sum')}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <Modal
          ...
          title={t('reception.selectDoctor')}
          width={600}
        >
          {doctors.length === 0 ? (
        ...
        <Modal
          ...
          title={t('reception.searchPatient')}
          width={600}
        >
            ...
            <div className="service-list">
              {existingPatients.map((p) => (
                <button
                  ...
                  style={{
                    width: '100%',
                    borderLeft: p.activeRoomId ? '4px solid var(--accent-warning)' : '',
                  }}
                  ...
                  <div style={{ fontWeight: 600 }}>
                    ...
                    {p.activeRoomId ? (
                      <span className="badge badge-warning" style={{ marginLeft: 8, fontSize: 10 }}>
                        {t('reception.statsionarBadge')}: {p.roomName}
                      </span>
                    ) : (
                      ''
                    )}
        ...
        {/* Check Preview Modal */}
        <Modal
          isOpen={showCheckPreview}
          onClose={() => setShowCheckPreview(false)}
          title={t('reception.checkPreview')}
          width={520}
        >
            ...
            <div className="check-preview-services">
              {selectedServices.map((svc) => (
                <div key={svc.id} className="check-preview-svc">
                  <span>
                    {svc.name} {svc.quantity > 1 ? `× ${svc.quantity}` : ''}
                  </span>
                  <span>
                    {formatPrice(svc.price * svc.quantity)} {t('common.sum')}
                  </span>
            ...
            {discount > 0 && (
              <div className="check-preview-row">
                <span className="check-label">
                  {t('reception.discount')} ({discount}%):
                </span>
                <span className="check-value" style={{ color: 'var(--accent-danger)' }}>
                  -{formatPrice(discountAmount)} {t('common.sum')}
                </span>
              </div>
            )}
        ...
        <Modal
          ...
          title={activeShift ? t('reception.closeShift') : t('reception.startShift')}
          width={450}
        >
            ...
            {activeShift ? (
              <>
                <div style={{ fontSize: 50, marginBottom: 15 }}>
                  {activeShift.type === 'day' ? '☀️' : '🌙'}
                </div>
                <p style={{ marginBottom: 10 }}>
                  <b>
                    {activeShift.type === 'day'
                      ? t('reception.dayShiftBtn')
                      : t('reception.nightShiftBtn')}
                  </b>{' '}
                  {t('reception.closeShiftConfirm')}
                </p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20 }}>
                  {t('reception.closeShiftHint')}
                </p>
              ...
              <div className="shift-summary-view">
                ...
                <h3 style={{ marginBottom: 15, color: 'var(--success)' }}>
                  {t('reception.shiftClosed2')}
                </h3>
                <div
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontWeight: 'bold' }}>
                      {formatPrice(shiftSummary.transfer)} som
                    </span>
                  </div>
                  <div className="check-preview-divider" />
                  <div
                ...
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 15 }}>
                  {t('reception.selectNewShift')}
                </p>
                <div className="form-actions" style={{ justifyContent: 'center', gap: 15 }}>
                  <button className="btn btn-success" onClick={() => startManualShift('day')}>
                    ☀️ {t('reception.dayShiftBtn')}
                  </button>
                  <button className="btn btn-warning" onClick={() => startManualShift('night')}>
                    🌙 {t('reception.nightShiftBtn')}
                  </button>
                </div>
              </div>
              ...
              <>
                ...
                <p style={{ marginBottom: 20 }}>
                  {t('reception.noActiveShift')}
                  <br />
                  {t('reception.startShiftHint')}
                </p>
                <div className="form-actions" style={{ justifyContent: 'center', gap: 15 }}>
                  <button className="btn btn-success" onClick={() => startManualShift('day')}>
                    ☀️ {t('reception.dayShiftFull')}
                  </button>
                  <button className="btn btn-warning" onClick={() => startManualShift('night')}>
                    🌙 {t('reception.nightShiftFull')}
                  </button>
                </div>
              </>
            )}
