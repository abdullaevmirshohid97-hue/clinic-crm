...
export default function Queue() {
  ...
  async function finalPrintTicket() {
    ...
    try {
      ...
      if (previewTicket.patientName) {
        const existing = await db.query('SELECT id FROM patients WHERE fullName = ? LIMIT 1', [previewTicket.patientName]);
        if (existing.length > 0) {
          patientId = existing[0].id;
        } else {
          const pResult = await db.insertReturn('patients', { fullName: previewTicket.patientName });
    ...
    return (
      <div className="queue-ticket">
        ...
        <div className="queue-ticket-info">
          ...
          <div className="queue-ticket-time">{new Date(ticket.createdAt).toLocaleTimeString('uz-UZ', { hour: '2-digit', minute: '2-digit' })}</div>
        </div>
  ...
  return (
    <div className="page page-queue">
      <div className="page-header">
        <div style={{display:'flex', alignItems:'center', gap: 20}}>
          ...
          <div className="btn-group">
            ...
            <button className="btn btn-special" style={{background: '#8b5cf6', color: '#fff'}} onClick={() => window.open('/?page=monitor', '_blank')}>📺 TV Mode (Voice)</button>
          </div>
        </div>
                    ...
                    <div className="queue-ticket-list">
                      {waiting.map(ticket => (
                        <TicketCard key={ticket.id} ticket={ticket} actions={
                          <button className="btn btn-sm btn-primary" onClick={() => updateStatus(ticket.id, 'in_progress')} title="Qabul qilish">Qabul qilish</button>
                        } />
                      ))}
                    </div>
                  )}
                  ...
                  {inProgress.length === 0 ? (
                    ...
                    <div className="queue-ticket-list">
                      {inProgress.map(ticket => (
                        <TicketCard key={ticket.id} ticket={ticket} actions={
                          <button className="btn btn-sm btn-success" onClick={() => updateStatus(ticket.id, 'completed')} title="Yakunlash">Yakunlash</button>
                        } />
                      ))}
                    </div>
                  )}
      ...
      <Modal isOpen={!!previewTicket} onClose={() => setPreviewTicket(null)} title="Navbat Cheki Namunasi" width={400}>
        {previewTicket && (
          <div className="ticket-preview-box" style={{textAlign:'center', padding: 20}}>
            ...
            <div className="form-actions" style={{justifyContent: 'center', gap: 15}}>
               <button className="btn btn-danger" onClick={() => setPreviewTicket(null)}>❌ Atkaz</button>
               <button className="btn btn-primary" onClick={finalPrintTicket}>🖨️ Chiqarish</button>
            </div>
          </div>
        )}
      </Modal>
