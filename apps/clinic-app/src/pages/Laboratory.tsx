...
export default function Laboratory() {
  ...
  }>({ id: null, testName: '', category: '', price: 0, fields: [] });
  ...
  function openEditTemplate(tpl: LabTestTemplate) {
    ...
    try {
      parsedFields = JSON.parse(tpl.fieldsJson || '[]') as TemplateFieldDraft[];
    } catch (_e) {
      /* invalid JSON */
    }
    setTemplateForm({
  ...
  async function openWriteResult(tst: LabTest) {
    ...
    try {
      const resultObj = JSON.parse(tst.resultJson || '{}') as {
        fields?: Array<{ name: string; value?: string }>;
        legacy?: string;
      };
      if (resultObj.fields) {
        resultObj.fields.forEach((f) => {
          existingData[f.name] = f.value ?? '';
        });
      }
  ...
  async function saveResultData() {
    ...
    try {
      ...
      if (tpl)
        try {
          tplFields = JSON.parse(tpl.fieldsJson || '[]') as TemplateFieldDraft[];
        } catch (_e) {
          /* invalid JSON */
        }
      ...
      for (const fDef of tplFields) {
        ...
        if (val !== '' && (fDef.type === 'number' || fDef.type === 'formula')) {
          ...
          min = String(
            gender === 'ayol' || gender === 'f' || gender === 'female'
              ? (fDef.minF ?? '')
              : (fDef.minM ?? '')
          );
          max = String(
  ...
  function printPDF(tst: LabTest) {
    ...
    try {
      resObj = JSON.parse(tst.resultJson || '{"fields":[]}');
    } catch (e) {
      /* invalid JSON */
    }
          ...
          ${
            ...
            <table class="result-table">
              ...
              ${resObj.fields
                .map((f) => {
                  let fLabel = '';
                  if (f.flag === 'LL') fLabel = 'Juda Past ⚠️';
                  else if (f.flag === 'L') fLabel = 'Past ↓';
                  else if (f.flag === 'H') fLabel = 'Baland ↑';
                  else if (f.flag === 'HH') fLabel = 'Juda Baland ⚠️';
                  return `
  ...
  if (activeTestTpl) {
    try {
      tplFields = JSON.parse(activeTestTpl.fieldsJson || '[]') as TemplateFieldDraft[];
    } catch (_e) {
      /* invalid JSON */
    }
  }
  ...
  return (
    <div className="page page-laboratory">
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {activeTab === 'workflow' ? (
            <button
              ...
              onClick={() => {
                setTestForm({ patientId: '', testName: '', isUrgent: false });
            ...
            <button
              className="btn btn-primary"
              onClick={() => {
                setTemplateForm({ id: null, testName: '', category: '', price: 0, fields: [] });
                setShowTemplateModal(true);
              }}
            >
              + Shablon (Konstruktor)
          ...
          >
            <button
              ...
              style={{
                ...
                background:
                  activeTab === 'workflow' ? 'var(--accent-primary-glow)' : 'var(--bg-input)',
                display: 'flex',
                gap: 15,
                alignItems: 'center',
                cursor: 'pointer',
              ...
              <div>
                <div
                  style={{
                    ...
                    fontSize: 15,
                    color:
                      activeTab === 'workflow' ? 'var(--accent-primary)' : 'var(--text-primary)',
                  }}
                >
                  Gibrid Ish Oqimi
                </div>
                <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>
                  Shtrix-kodlarni biriktirish va mashina natijalarini ulash.
                </div>
              </div>
            </button>
            <button
              ...
              <div>
                <div
                  style={{
                    ...
                    fontSize: 15,
                    color:
                      activeTab === 'templates' ? 'var(--accent-success)' : 'var(--text-primary)',
                  }}
                >
                  Shablon Konstruktori
                </div>
                <div style={{ fontSize: 12, opacity: 0.7, marginTop: 4 }}>
                  Demografik normativlar va analiz parametrlarini yaratish.
                </div>
              </div>
            </button>
          </div>
        ...
        <div className="settings-content" style={{ flex: 1, minWidth: 0 }}>
            ...
            >
              {columns.map((col) => {
                ...
                return (
                  <div
                    key={col.id}
                    className="kanban-col card glass-card"
                    style={{
                      borderTop: `0px solid ${col.color}`,
                      overflow: 'hidden',
                      display: 'flex',
                  ...
                  >
                    ...
                    >
                      {colTests.map((tst) => (
                        <div
                          key={tst.id}
                          className="kanban-card p-3 bg-input"
                          style={{
                            borderRadius: 8,
                        ...
                        >
                          ...
                          <div
                            style={{
                              fontWeight: 800,
                              fontSize: 14,
                              paddingRight: tst.isUrgent === 1 ? 40 : 0,
                            }}
                          >
                            {tst.patientName}
                          </div>
                          ...
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>
                            {col.id === 'pending' && (
                              <button
                                ...
                                onClick={() => updateTestStatus(tst.id, 'sampled')}
                              >
                                Namuna olindi →
                              </button>
                            )}
                            {col.id === 'sampled' && (
                              <button
                                ...
                                onClick={() => updateTestStatus(tst.id, 'processing')}
                              >
                                Jarayonga ishga →
                              </button>
                            )}
                            ...
                            {col.id === 'ready' && (
                              <div style={{ display: 'flex', gap: 6, flex: 1 }}>
                                ...
                                <button
                                  ...
                                  style={{ flex: 8, borderColor: 'var(--accent-success)' }}
                                  onClick={() => printPDF(tst)}
                                >
                                  🖨️ PDF Chop etish
                                </button>
                              </div>
          ...
          {activeTab === 'templates' && (
            <div className="card glass-card">
              <div
                className="p-3 bg-primary-glow"
                style={{ borderBottom: '1px solid var(--border-color)', fontSize: 13 }}
              >
                <b>LIMS Shablonlar:</b> Bu parametrlar orqali har bir analizning ichki
                ko'rsatkichlari tuziladi. Bemorga qog'oz chiqarayotganda ushbu qismlar avtomatik
                jadvallashadi.
              </div>
                  ...
                  <tbody>
                    {templates.map((tpl) => {
                      ...
                      return (
                        <tr key={tpl.id}>
                          <td>
                            <div style={{ fontWeight: 600 }}>{tpl.testName}</div>
                          </td>
                          <td>
                            <span className="badge badge-outline">{tpl.category || '—'}</span>
                          </td>
                          <td style={{ fontWeight: 700 }}>{f.length} ta xususiyat</td>
                          <td style={{ color: 'var(--accent-success)' }}>
                            {Number(tpl.price || 0).toLocaleString()} so'm
                          </td>
                          <td>
                            <button
      ...
      >
        <div className="p-4" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          >
            <input
              type="checkbox"
              id="urgentCheck"
              checked={testForm.isUrgent}
              onChange={(e) => setTestForm({ ...testForm, isUrgent: e.target.checked })}
            />
            <label
              htmlFor="urgentCheck"
              style={{ margin: 0, color: 'var(--accent-danger)', fontWeight: 800 }}
            >
              CITO! (Shoshilinch tahlil)
            </label>
          </div>
        </div>
        <div
          className="form-actions p-4"
          style={{
        ...
        >
          ...
          <button className="btn btn-primary" onClick={saveTest}>
            Boshlash va Shtrix-kod ajratish
          </button>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title="LIMS Shablon Konstruktori (Form Builder)"
        width={1000}
      >
        <div
          className="p-4"
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 12 }}>
            <div className="form-group">
              ...
              <input
                type="text"
                className="form-input"
                value={templateForm.testName}
                onChange={(e) => setTemplateForm({ ...templateForm, testName: e.target.value })}
              />
            </div>
            <div className="form-group">
              <label>Bo'lim</label>
              <input
                ...
                placeholder="Gematologiya, Bioximya..."
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setTemplateForm({ ...templateForm, price: parseFloat(e.target.value) })
                }
              />
            </div>
          </div>
          ...
          >
            Ichki Parametrlar (Referens/Norma)
          </h4>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div className="form-group" style={{ flex: 2 }}>
                  ...
                  <input
                    ...
                    className="form-input"
                    value={f.name || ''}
                    onChange={(e) => updateTemplateField(f.id, 'name', e.target.value)}
                    placeholder="WBC, Gemoglobin..."
                  />
                </div>
                <div className="form-group" style={{ flex: 1 }}>
                  ...
                  <select
                    className="form-input"
                    value={f.type || 'number'}
                    onChange={(e) => updateTemplateField(f.id, 'type', e.target.value)}
                  >
                ...
                <button
                  ...
                  style={{ marginTop: 20 }}
                  onClick={() => removeTemplateField(f.id)}
                >
                  ✕
                </button>
              </div>
              ...
              {f.type === 'formula' && (
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-input"
                    value={f.formula || ''}
                    onChange={(e) => updateTemplateField(f.id, 'formula', e.target.value)}
                    placeholder="Matematik formula..."
                  />
                </div>
              )}
              ...
              {f.type === 'select' && (
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-input"
                    value={f.options || ''}
                    onChange={(e) => updateTemplateField(f.id, 'options', e.target.value)}
                    placeholder="Musbat, Manfiy, Shubhali..."
                  />
                </div>
              )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                  <div className="form-group">
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      <input
                        ...
                        className="form-input"
                        value={f.minM || f.min || ''}
                        onChange={(e) => updateTemplateField(f.id, 'minM', e.target.value)}
                        placeholder="Min"
                      />
                      <input
                        type="text"
                        className="form-input"
                        value={f.maxM || f.max || ''}
                        onChange={(e) => updateTemplateField(f.id, 'maxM', e.target.value)}
                        placeholder="Max"
                      />
                    </div>
                  </div>
                  <div className="form-group">
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      <input
                        ...
                        className="form-input"
                        value={f.minF || f.min || ''}
                        onChange={(e) => updateTemplateField(f.id, 'minF', e.target.value)}
                        placeholder="Min"
                      />
                      <input
                        type="text"
                        className="form-input"
                        value={f.maxF || f.max || ''}
                        onChange={(e) => updateTemplateField(f.id, 'maxF', e.target.value)}
                        placeholder="Max"
                      />
                    </div>
                  </div>
                  <div className="form-group">
                    ...
                    <div style={{ display: 'flex', gap: 5 }}>
                      <input
                        ...
                        className="form-input"
                        value={f.panicL || ''}
                        onChange={(e) => updateTemplateField(f.id, 'panicL', e.target.value)}
                        placeholder="LL"
                      />
                      <input
                        type="text"
                        className="form-input"
                        value={f.panicH || ''}
                        onChange={(e) => updateTemplateField(f.id, 'panicH', e.target.value)}
                        placeholder="HH"
                      />
                    </div>
                  </div>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      className="form-input"
                      value={f.unit || ''}
                      onChange={(e) => updateTemplateField(f.id, 'unit', e.target.value)}
                      placeholder="mg/dl"
                    />
                  </div>
                </div>
      ...
      <Modal
        ...
        title="Natijalarni Kiritish & Tasdiqlash"
        width={1000}
      >
        {activeTest && (
          <div
            ...
            {tplFields.length > 0 ? (
                  ...
                  <tbody>
                    {tplFields.map((f) => {
                        ...
                        try {
                          ...
                          tplFields.forEach((otherF) => {
                            if (otherF.name !== f.name) {
                              ...
                              const otherVal = parseFloat(
                                Array.isArray(rv) ? '0' : String(rv ?? 0)
                              );
                              expr = expr.replace(
                                new RegExp(`\\[${otherF.name}\\]`, 'g'),
                                String(otherVal)
                              );
                            }
                          });
                      ...
                      const historicalFields = Array.isArray(resultData['__historicalFields'])
                        ...
                        : [];
                      let deltaHtml = null;
                      const hF = historicalFields.find((hf) => hf.name === f.name) as
                        | (LabTestTemplateField & { value?: string })
                        | undefined;
                      ...
                      return (
                        <tr key={f.name}>
                          ...
                          <td style={{ padding: '8px 5px' }}>
                            {f.type === 'select' ? (
                              <select
                                ...
                                value={val}
                                onChange={(e) =>
                                  setResultData({ ...resultData, [f.name]: e.target.value })
                                }
                              >
                                <option value="">Tanlang...</option>
                              ...
                              <input
                                ...
                                className="form-input"
                                style={{
                                  borderColor: flag !== 'N' ? 'var(--accent-danger)' : '',
                                  background: flag !== 'N' ? 'rgba(220,38,38,0.1)' : '',
                                }}
                                value={val}
                                onChange={(e) =>
                                  setResultData({ ...resultData, [f.name]: e.target.value })
                          ...
                          <td style={{ padding: '8px 0', fontSize: 12 }}>
                            {deltaHtml || <span style={{ opacity: 0.4 }}>—</span>}
                          </td>
                          <td
                            style={{ padding: '8px 0', fontSize: 12, color: 'var(--text-muted)' }}
                          >
                            {f.type === 'number' || f.type === 'formula'
                              ? `${min || '?'} - ${max || '?'} (${f.unit})`
                              : '—'}
              ...
              <div className="form-group mt-2">
                ...
                <textarea
                  ...
                  rows={6}
                  value={(() => {
                    const v = resultData['__legacy'];
                    return Array.isArray(v) ? '' : String(v ?? '');
                  })()}
                  onChange={(e) => setResultData({ ...resultData, __legacy: e.target.value })}
                  placeholder="Xulosa..."
                ></textarea>
              </div>
            )}
        ...
        >
          ...
          <button className="btn btn-success" onClick={saveResultData}>
            Tasdiqlash & Tayyorlash
          </button>
        </div>
      </Modal>
    </div>
