...
function evaluateMathExpression(rawExpr: string): number | null {
      ...
      tplFields.forEach(f => {
        if (f.type === 'formula' && f.formula) {
           try {
              ...
              const calculated = evaluateMathExpression(expr.replace(/[^-+*/().0-9]/g, ''));
              if (calculated !== null) resolvedData[f.name] = calculated.toFixed(2);
                          ...
                          {tplFields.map(f => {
                             ...
                             if (f.type === 'formula' && f.formula) {
                                try {
                                   ...
                                   const calculated = evaluateMathExpression(expr.replace(/[^-+*/().0-9]/g, ''));
                                   if (calculated !== null) currentVal = calculated.toFixed(2);
