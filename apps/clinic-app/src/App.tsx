...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => {
    try {
      return localStorage.getItem('clinic_theme') || 'dark';
    } catch {
      return 'dark';
    }
  });
    ...
    return (
      ...
      >
        ...
        <p style={{ color: '#94a3b8', marginBottom: 24, maxWidth: 480, lineHeight: 1.6 }}>
          ...
          <code
            style={{ background: '#1e293b', padding: '2px 6px', borderRadius: 4, color: '#38bdf8' }}
          >
            .env
          </code>{' '}
          faylini yarating va Supabase credentials ni kiriting.
        </p>
        <div
          style={{
            background: '#1e293b',
        ...
        >
          <div>
            1. <strong style={{ color: '#e2e8f0' }}>.env.example</strong> →{' '}
            <strong style={{ color: '#e2e8f0' }}>.env</strong> nusxa oling
          </div>
          <div>
            2. <strong style={{ color: '#e2e8f0' }}>VITE_SUPABASE_URL</strong> ni to'ldiring
          </div>
          <div>
            3. <strong style={{ color: '#e2e8f0' }}>VITE_SUPABASE_ANON_KEY</strong> ni to'ldiring
          </div>
  ...
  if (authState.isLoading) {
    return (
      ...
      >
        <div
          className="spinner"
          style={{ width: 40, height: 40, borderTopColor: 'var(--primary)' }}
        ></div>
      </div>
    );
  }
